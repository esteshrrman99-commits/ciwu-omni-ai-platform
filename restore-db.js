...
(async () => {
  ...
  // Batch 1: Nanomedicine (15 entities)
  const nanomedBatch = [
  ...
  // Batch 2: Synthetic Biology (15 entities)
  const synthBioBatch = [
  ...
  // Batch 3: Brain Computer Interface (15 entities)
  const bciBatch = [
  ...
  // Batch 4: Space Medicine (15 entities)
  const spaceMedBatch = [
  ...
  // Batch 5: Quantum Biology (15 entities)
  const quantumBioBatch = [
  ...
  // Batch 6: Biocompatible Materials (15 entities)
  const bioMatBatch = [
  ...
  // Batch 7: Digital Health (15 entities)
  const digitalHealthBatch = [
  ...
  // Batch 8: CIWU Core Systems (15 entities)
  const ciwuCoreBatch = [
  ...
  // Batch 9: Longevity Connections (15 entities)
  const longevityBatch = [
  ...
  // Batch 10: CIWU AI Core (15 entities)
  const aiCoreBatch = [
  ...
  // Insert all batches
  let count = 0;
  [...nanomedBatch, ...synthBioBatch, ...bciBatch, ...spaceMedBatch, ...quantumBioBatch, ...bioMatBatch, ...digitalHealthBatch, ...ciwuCoreBatch, ...longevityBatch, ...aiCoreBatch].forEach(([entity, relation, value]) => {
    db.run(`INSERT INTO knowledge (entity, relation, value) VALUES (?, ?, ?)`, [entity, relation, value]);
    count++;
  });
  
  // Insert entities into EONS
  const entities = [
    ["Mother_Profile", "patient", "Primary_family_member_longevity_target"],
    ["Wife_Profile", "patient", "Secondary_family_member_longevity_target"],
    ["Children_Profile", "patient", "Generational_protection_targets"],
    ["Father_Profile", "patient", "Elder_care_longevity_target"],
    ["Neighbor_Profile", "patient", "Extended_community_protection"],
    ["Zortex_Matrix", "technology", "Advanced_cellular_architecture_system"],
    ["Neurotex_Core", "technology", "Neural_connectivity_optimization"],
    ["Vortex_Engine", "technology", "Core_cell_regeneration_unit"],
    ["Cortex_Knowledge", "database", "1763_entities_909_relations"],
    ["Eons_Relations", "database", "Complete_knowledge_graph"],
    ["Medical_Record_Uploader", "feature", "Document_scanning_pipeline"],
    ["DNA_Analyzer", "feature", "Gene_optimization_algorithm"],
    ["Supplement_Formulator", "feature", "Cost_efficient_therapy_creator"],
    ["Escrow_Security", "system", "Family_fund_protection"],
    ["Health_Scout", "agent", "Real_time_medical_intelligence"]
  ];
  
  entities.forEach(([name, type, desc]) => {
    eonsDb.run(`INSERT INTO entities (name, type, description) VALUES (?, ?, ?)`, [name, type, desc]);
  });
  
  // Insert relations
  const relations = [
    ["Mother_Profile", "needs", "Zortex_Matrix"],
    ["Wife_Profile", "needs", "Neurotex_Core"],
    ["Children_Profile", "protected_by", "Vortex_Engine"],
    ["Father_Profile", "treated_by", "Cortex_Knowledge"],
    ["Medical_Record_Uploader", "feeds", "DNA_Analyzer"],
    ["DNA_Analyzer", "generates", "Supplement_Formulator"],
    ["Escrow_Security", "protects", "Family_funds"],
    ["Health_Scout", "monitors", "All_family_members"]
  ];
  
  relations.forEach(([source, rel, target]) => {
    eonsDb.run(`INSERT INTO relations (source, target, relation_type) VALUES (?, ?, ?)`, [source, target, rel]);
  });
  ...
  console.log(`✅ RESTORED: ${count} knowledge facts`);
  console.log(`✅ EONS: ${entities.length} entities, ${relations.length} relations`);
